...
しかし、運用を想定していくうちに、ある課題が見えてきました。

> 「このお客様、前回の物件は見送りになったけど、今回また別の物件で問い合わせてきた」

> 「同じお客様がファミリー向けと単身向け、2件並行で検討している」

実際の不動産仲介業務では、**1人のお客様が複数の案件を抱える**ことは珍しくありません。前回の物件は失注したけど、今回は別の物件で商談中——そんなケースで、顧客のステータスを1つしか持てないのは不便です。
...
├── Inquiry（案件1：物件Aの商談）
│   ├── deal_status: 失注
│   ├── PropertyInquiry（物件Aへの問い合わせ）
│   └── CustomerActivity（案件1の対応履歴）
│
└── Inquiry（案件2：物件Bの商談）
    ├── deal_status: 内見予約
    ├── PropertyInquiry（物件Bへの問い合わせ）
    └── CustomerActivity（案件2の対応履歴）
```

**案件（Inquiry）** という中間レイヤーを設けることで、1人のお客様に対して複数の商談を独立して管理できるようになりました。
...
物件への問い合わせ（PropertyInquiry）は、必ず案件（Inquiry）に紐付くようになりました。
...
公開ページからの問い合わせフォーム送信時には、Inquiry が自動的に作成されます。
...
1. 案件内に複数の PropertyInquiry が紐付く
2. 「ファミリー向け」「単身向け」など、案件ごとにメモを残せる
3. 片方が失注しても、もう片方の商談は影響を受けない
...
今回の再設計で、CoCoスモの顧客管理は **「1顧客 = 1ステータス」から「1顧客 × N案件」** へと進化しました。

- **Inquiry モデル**が商談の進捗を管理
- **Customer モデル**は顧客の「人」としての情報に集中
- **PropertyInquiry**は案件に紐付く物件単位の問い合わせ
- **CustomerActivity**は案件ごとの対応履歴

データ構造が業務の実態に近づいたことで、より自然な顧客対応フローが実現できます。

「このお客様、前の案件では失注だったけど、今回は条件が合いそうだ」——そんな営業判断を、データが支えます。
  MARKDOWN
  post.status = :published
