  ...
  return (
    <Box className="template2-roomspot print-container" style={{ '--primary-color': colors.primary, '--accent-color': colors.accent }}>
      ...
      <style>{`
        ...
        .template2-roomspot .access-preview {
          display: flex;
          gap: 16px;
          flex-wrap: wrap;
          margin-bottom: 24px;
        }
        .template2-roomspot .access-preview-item {
          display: flex;
          align-items: center;
          gap: 6px;
          font-size: 13px;
        ...
        .template2-roomspot .access-line-badge {
          color: #fff;
          padding: 1px 6px;
          border-radius: 3px;
          font-size: 11px;
        ...
        .template2-roomspot .pr-text p { margin: 0 0 0.5em 0; }
        .template2-roomspot .pr-text h2 { font-size: 1.25rem; font-weight: 600; margin: 1em 0 0.5em 0; }
        .template2-roomspot .pr-text h3 { font-size: 1.1rem; font-weight: 600; margin: 1em 0 0.5em 0; }
        .template2-roomspot .pr-text ul, .template2-roomspot .pr-text ol { padding-left: 1.5em; margin: 0.5em 0; }
        .template2-roomspot .pr-text li { margin: 0.25em 0; }
        .template2-roomspot .pr-text blockquote { border-left: 3px solid var(--primary-color); padding-left: 1em; margin: 0.5em 0; color: #666; font-style: italic; }
        .template2-roomspot .pr-text a { color: var(--accent-color); text-decoration: underline; }
        .template2-roomspot .vr-section { margin-bottom: 32px; }
        .template2-roomspot .vr-item { margin-bottom: 24px; }
        .template2-roomspot .vr-item-title { font-size: 16px; font-weight: bold; color: var(--accent-color); margin-bottom: 8px; }
        .template2-roomspot .vr-item-description { font-size: 13px; color: #666; margin-bottom: 12px; line-height: 1.6; }
        .template2-roomspot .vr-iframe { width: 100%; height: 550px; border: 2px solid #e0e0e0; border-radius: 8px; }
        .template2-roomspot .share-box { background: white; border: 2px solid #e0e0e0; border-radius: 8px; padding: 20px; }
        .template2-roomspot .share-title { font-size: 14px; font-weight: bold; color: #333; margin-bottom: 12px; }
        .template2-roomspot .sections-container { margin-bottom: 32px; }
        @media (max-width: 960px) {
          .template2-roomspot .header-section { padding: 12px 0; }
          .template2-roomspot .header-logo { font-size: 18px; }
          .template2-roomspot .header-badge { font-size: 10px; padding: 5px 12px; }
          .template2-roomspot .property-title { font-size: 20px; }
          .template2-roomspot .two-column { grid-template-columns: 1fr; }
          .template2-roomspot .details-table th { width: 120px; font-size: 13px; }
          .template2-roomspot .details-table td { font-size: 13px; }
          .template2-roomspot .quick-info { grid-template-columns: 1fr; }
        }
      `}</style>
      ...
      <Container maxWidth="lg" className="main-container">
        ...
        {visibleFields.access_section && accessPreview?.length > 0 && (
          <div className="access-preview">
            {accessPreview.map((bs) => (
              <div key={bs.id} className="access-preview-item">
                <TrainIcon sx={{ fontSize: 16, color: bs.station?.railway_line?.color || colors.primary }} />
                {bs.station?.railway_line?.name && (
                  <span className="access-line-badge" style={{ backgroundColor: bs.station.railway_line.color || colors.primary }}>
                    {bs.station.railway_line.name}
                  </span>
                )}
          ...
          <div>
            {visibleFields.rent && room.rent && (
              <div className="price-box">
                ...
                <div className="price-amount">&yen;{room.rent.toLocaleString()}</div>
        ...
        {pr_text && (
          <div>
            ...
            <div className="pr-text" style={{ whiteSpace: 'normal' }} dangerouslySetInnerHTML={{ __html: pr_text }} />
          </div>
        )}

        {/* 物件概要 */}
        <div id="property-info">
          <h2 className="section-title">物件概要</h2>
          <table className="details-table">
            <tbody>
              ...
              {visibleFields.built_year && building?.built_year && (
                <tr><th>築年月</th><td>{building.built_year}年</td></tr>
              )}
        ...
        {/* 部屋の情報 */}
        <div>
          <h2 className="section-title">部屋の情報</h2>
          <table className="details-table">
            <tbody>
              ...
              {visibleFields.floor && room.floor && (
                <tr><th>所在階</th><td>{room.floor}階</td></tr>
              )}
              ...
              <div>
                {property_publication_vr_tours.map((item) => (
                  <div key={item.vr_tour.id} className="vr-item">
                    ...
                    <iframe src={`/vr/${item.vr_tour.public_id}`} className="vr-iframe" title={item.vr_tour.title} />
                  </div>
                ))}
              </div>
            )}
              ...
              <div>
                {property_publication_virtual_stagings.map((item) => (
                  <div key={item.virtual_staging.id} className="vr-item">
                    ...
                    <iframe src={`/virtual-staging/${item.virtual_staging.public_id}?embed=true`} className="vr-iframe" title={item.virtual_staging.title} />
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="share-box no-print">
          ...
          <ShareButtons
            ...
            publicationId={publicationId}
          />
          <PdfExportButton title={title} publicationId={publicationId} size="large" />
        </div>
      </Container>
    </Box>
  );
